...
```{r,include=FALSE, eval=FALSE}
saveRDS(ar1trend_mod, here::here("saved-objects/ar1trend_mod.rds"))
```

```{r,include=FALSE, eval=TRUE,cache=TRUE}
ar1trend_mod = readRDS(here::here("saved-objects/ar1trend_mod.rds"))
```
...
```{r}
plot_mvgam_smooth(ar1trend_mod)
```
...
sapply(1:5, function(x) plot_mvgam_trend(ar1trend_mod, series = x))
```
...
```{r}
plot_predictions(ar1trend_mod, condition = c("temp"), conf_level = 0.9)
```

Next, the partial effect of seasonality (here, months) on plankton counts across groups: 

```{r}
plot_predictions(ar1trend_mod, condition = c("month"), conf_level = 0.9)
```

And finally, the effect of different plankton groups on the model:

```{r}
plot_predictions(ar1trend_mod, condition = c("series"), conf_level = 0.9)
```


##### Calculating the rate of change in plankton counts
...
```{r, fig.ncol=2, message=F, warning=F, results = 'hide'}
sapply(1:5, function(x) plot_mvgam_trend(ar1trend_mod, series = x, derivatives = TRUE))
```
...
species_correlations = lv_correlations(ar1trend_mod)
...
We will use leave-one-out comparison to compare the two models we just built: `notrend_mod` which does not have a dynamic trend, and `ar1trend` which includes an autoregressive latent trend model.

This workshop isn't really about model comparison, so we will just use this approach as an example, but there are other ways you could compare these models.  metric of comparison will be expected log pointwise predictive density (ELPD)

```{r}
mvgam::loo_compare(notrend_mod, ar1trend_mod,
                   model_names = c("No trend", "AR1 trend"))
```

The AR1 trend model performs better than the model without a dynamic trend (here, we want high values for elpd_dif)
